...
import { countdownLabel, daysUntilNextOccurrence, formatMonthDay } from "@/lib/date";
import { cn } from "@/lib/utils";

type Person = components["schemas"]["Person"];

export function AnniversaryCard({ person }: { person: Person }): React.ReactElement {
  const days = daysUntilNextOccurrence(person.anniversary);
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          {person.anniversary ? formatMonthDay(person.anniversary) : "Anniversary"}
        </p>
      </div>
      <span
        className={cn(
